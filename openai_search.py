...
You previously attempted to describe products for the query "{query}" but the
response was not valid JSON. Reformat the content below into ONLY a JSON array
where every element is an object containing the keys: title (string), price
(number), in_stock (boolean), source (string), link (string URL), and image
(string URL). Do not include any commentary or markdown fences.
...
def _request_reformat(query: str, content: str) -> List[Dict[str, Any]]:
    """Ask OpenAI to reformat *content* into the expected JSON layout."""
    ...
    reformat_messages = [
        {
            ...
            "content": (
                ...
                "information for repair parts and devices. Always answer in "
                "strict JSON when asked to reformat content."
            ),
        },
        {
            "role": "user",
            "content": REFORMAT_PROMPT_TEMPLATE.format(query=query.strip(), content=content.strip()),
        },
    ]

    try:
        reformatted = _invoke_openai(reformat_messages)
    except Exception:  # pragma: no cover - defensive logging
        logger.exception("OpenAI reformat request failed")
        return []

    return _extract_json_array(reformatted)
...
def search_openai(query: str) -> List[Dict[str, Any]]:
    ...
    if not results:
        results = _request_reformat(query, raw_text)
